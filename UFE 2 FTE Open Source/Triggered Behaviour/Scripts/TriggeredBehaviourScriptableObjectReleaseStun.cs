...
namespace UFE2FTE
{
    [CreateAssetMenu(menuName = "U.F.E. 2 F.T.E./Triggered Behaviour/Release Stun", fileName = "New Release Stun Triggered Behaviour")]
    public class TriggeredBehaviourScriptableObjectReleaseStun : TriggeredBehaviourScriptableObject
    {
        [SerializeField]
        private bool releaseStun;
        [SerializeField]
        ...
        private void CheckTargetOptions(ControlsScript player, Fix64 delayActionTime)
        {
            ...
            if (TriggeredBehaviour.IsStringMatch(player.myInfo.characterName, targetOptions.excludedCharacterNameArray) == false)
            {
                ...
                {
                    ReleaseStun(player, delayActionTime);
                }
                ...
                if (targetOptions.useAllPlayerAssists == true)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        ReleaseStun(player.assists[i], delayActionTime);
                    }
                }
            }
            ...
            if (player.opControlsScript != null)
            {
                if (TriggeredBehaviour.IsStringMatch(player.opControlsScript.myInfo.characterName, targetOptions.excludedCharacterNameArray) == false)
                {
                    ...
                    {
                        ReleaseStun(player.opControlsScript, delayActionTime);
                    }
                    ...
                    if (targetOptions.useAllOpponentAssists == true)
                    {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            ReleaseStun(player.opControlsScript.assists[i], delayActionTime);
                        }
                    }
                }
            }
        ...
        private void ReleaseStun(ControlsScript player, Fix64 delayActionTime)
        {
            if (player == null)
            {
                return;
            ...
            {
                //UFE.DelaySynchronizedAction(player.ReleaseStun, delayActionTime);
                player.ReleaseStun();
            }
            else
            {
