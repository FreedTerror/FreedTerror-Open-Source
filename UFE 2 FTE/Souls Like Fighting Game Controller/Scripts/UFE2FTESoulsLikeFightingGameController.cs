...
namespace UFE2FTE
{
    public class UFE2FTESoulsLikeFightingGameController : UFEBehaviour, UFEInterface
    {
        ...
        {
            UFE2FTEHelperMethodsManager.SetGaugePointsPercent(UFE.GetPlayer1ControlsScript(), staminaOptions.staminaGaugeId, 100);

            UFE2FTEHelperMethodsManager.SetGaugePointsPercent(UFE.GetPlayer2ControlsScript(), staminaOptions.staminaGaugeId, 100);

            UFE2FTEHelperMethodsManager.SetGaugePointsPercent(UFE.GetPlayer1ControlsScript(), parryOptions.parryGaugeId, 100);

            UFE2FTEHelperMethodsManager.SetGaugePointsPercent(UFE.GetPlayer2ControlsScript(), parryOptions.parryGaugeId, 100);
        }
        ...
        private void SetParryOptionsOnButton(ButtonPress button, ControlsScript player)
        {
                ...
                && player.isBlocking == true)
            {
                ...
                UFE2FTEHelperMethodsManager.SetGaugePointsPercent(player, parryOptions.parryGaugeId, 0);
            }
        }
                    ...
                    for (int b = 0; b < lengthB; b++)
                    {
                            ...
                            for (int c = 0; c < lengthC; c++)
                            {
                                switch (gaugeAmountOptions[i].gaugeGainType)
                                {
                                    case GaugeAmountOptions.GaugeGainType.Set:
                                        UFE2FTEHelperMethodsManager.SetGaugePointsPercent(player, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;

                                    case GaugeAmountOptions.GaugeGainType.Add:
                                        UFE2FTEHelperMethodsManager.AddGaugePointsPercent(player, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;
                                }
                            }
                        }
                        ...
                        if (gaugeAmountOptions[i].usePlayerOpponent == true)
                        {
                            ...
                            for (int c = 0; c < lengthC; c++)
                            {
                                ...
                                {
                                    case GaugeAmountOptions.GaugeGainType.Set:
                                        UFE2FTEHelperMethodsManager.SetGaugePointsPercent(player.opControlsScript, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;

                                    case GaugeAmountOptions.GaugeGainType.Add:
                                        UFE2FTEHelperMethodsManager.AddGaugePointsPercent(player.opControlsScript, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;
                                }
                            }
                        }
        ...
        private void SetStaminaGauge(ControlsScript player)
        {
            ...
            if (player.currentMove == null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    if (player.currentBasicMove != staminaOptions.noStaminaGaugePercentAmountBasicMoves[i])
                    {
                        continue;
                    }

                    UFE2FTEHelperMethodsManager.AddGaugePointsPercent(player, staminaOptions.staminaGaugeId, staminaOptions.passiveStaminaGaugePercentAmount);
        ...
        private void SetParryGauge(ControlsScript player)
        {
            ...
            UFE2FTEHelperMethodsManager.AddGaugePointsPercent(player, parryOptions.parryGaugeId, parryOptions.passiveParryGaugePercentAmount);
        }
        ...
        private void SetParryConfirmOnMove(MoveInfo move, ControlsScript player)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                UFE2FTEHelperMethodsManager.HitPause(player, parryOptions.parryConfirmFreezingTime);

                UFE2FTEHelperMethodsManager.HitPause(player.opControlsScript, parryOptions.parryConfirmFreezingTime);

                if (player.playerNum == 1)
                {
                    if (parryOptions.useKillAllPlayerMovesOnParryConfirmMove == true)
                    {
                        UFE.DelaySynchronizedAction(UFE2FTEHelperMethodsManager.KillAllMovesPlayer1, parryOptions.parryConfirmFreezingTime);
                    }
                ...
                else if (player.playerNum == 2)
                {
                    if (parryOptions.useKillAllPlayerMovesOnParryConfirmMove == true)
                    {
                        UFE.DelaySynchronizedAction(UFE2FTEHelperMethodsManager.KillAllMovesPlayer2, parryOptions.parryConfirmFreezingTime);
                    }
        ...
        private void PlayParryConfirmReactBasicMovePlayer1()
        {
            ...
            UFE2FTEHelperMethodsManager.KillAllMoves(UFE.GetPlayer1ControlsScript());
            ...
            switch (UFE.GetPlayer1ControlsScript().currentState)
            {
                case PossibleStates.Stand:
                    ...
                    UFE2FTEHelperMethodsManager.ResetForces(UFE.GetPlayer1ControlsScript(), true, true, true);
                    break;
                ...
                case PossibleStates.BackJump:
                    ...
                    UFE2FTEHelperMethodsManager.ForceGrounded(UFE.GetPlayer1ControlsScript(), 2);
        ...
        private void PlayParryConfirmReactBasicMovePlayer2()
        {
            ...
            UFE2FTEHelperMethodsManager.KillAllMoves(UFE.GetPlayer2ControlsScript());
            ...
            switch (UFE.GetPlayer2ControlsScript().currentState)
            {
                case PossibleStates.Stand:
                    ...
                    UFE2FTEHelperMethodsManager.ResetForces(UFE.GetPlayer2ControlsScript(), true, true, true);
                    break;
                ...
                case PossibleStates.BackJump:
                    ...
                    UFE2FTEHelperMethodsManager.ForceGrounded(UFE.GetPlayer2ControlsScript(), 2);
        ...
        private void SetBlockBreakOnBlock(ControlsScript player)
        {
            if (blockBreakOptions.useBlockBreakOnBlockOn0Stamina == true)
            {
                    ...
                    && player.Physics.freeze == false)
                {
                    ...
                    UFE2FTESoulsLikeFightingGameEventsManager.CallOnBlockBreak(player);
                }
        ...
        private void SetBlockBreakOnParry(ControlsScript player)
        {
            ...
            {
                if (player.currentBasicMove == BasicMoveReference.ParryHigh
                    && player.Physics.freeze == false)
                {
                    if (blockBreakOptions.useResetPlayerStunOnParryBlockBreak == true)
                    {
                ...
                else if (player.currentBasicMove == BasicMoveReference.ParryLow
                    && player.Physics.freeze == false)
                {
                    if (blockBreakOptions.useResetPlayerStunOnParryBlockBreak == true)
                    {
                ...
                else if (player.currentBasicMove == BasicMoveReference.ParryCrouching
                    && player.Physics.freeze == false)
                {
                    if (blockBreakOptions.useResetPlayerStunOnParryBlockBreak == true)
                    {
                ...
                else if (player.currentBasicMove == BasicMoveReference.ParryAir
                    && player.Physics.freeze == false)
                {
                    if (blockBreakOptions.useResetPlayerStunOnParryBlockBreak == true)
                    {
        ...
        private void PlayBlockBreakBasicMove(ControlsScript player, BasicMoveReference basicMove = BasicMoveReference.Idle, bool dontPlayIfBlockBreakIsPlaying = false)
        {
            ...
            UFE2FTEHelperMethodsManager.KillAllMoves(player);
            ...
            switch (basicMove)
            {
                ...
                case BasicMoveReference.BlockingAirHit:
                    ...
                    UFE2FTEHelperMethodsManager.ForceGrounded(player, 2);

                    UFE2FTEHelperMethodsManager.ResetForces(player, true, true, true);
                    break;
                ...
                for (int a = 0; a < lengthA; a++)
                {
                    ...
                    switch (characterShakeOptions[i].characterShakeStartMode)
                    {
                        case CharacterShakeOptions.CharacterShakeStartMode.EveryFrame:                          
                            UFE2FTEHelperMethodsManager.ShakeCharacterPosition(player, characterShakeOptions[i].characterShakeOffset);
                            break;
