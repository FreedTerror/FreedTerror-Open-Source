...
namespace FreedTerror.UFE2
{
    public class SoulsLikeFightingGameController : UFEBehaviour, UFEInterface
    {
        ...
        {
            SetStopGaugeDrain(UFE.p1ControlsScript);

            SetStopGaugeDrain(UFE.p2ControlsScript);

            SetStaminaGauge(UFE.p1ControlsScript);

            SetStaminaGauge(UFE.p2ControlsScript);

            SetParryGauge(UFE.p1ControlsScript);

            SetParryGauge(UFE.p2ControlsScript);

            SetBlockBreakOnBlock(UFE.p1ControlsScript);

            SetBlockBreakOnBlock(UFE.p2ControlsScript);

            SetBlockBreakOnParry(UFE.p1ControlsScript);

            SetBlockBreakOnParry(UFE.p2ControlsScript);

            SetCharacterShakeOptions(UFE.p1ControlsScript);

            SetCharacterShakeOptions(UFE.p2ControlsScript);
        }
        ...
        {
            UFE2Manager.SetGaugePointsPercent(UFE.p1ControlsScript, staminaOptions.staminaGaugeId, 100);

            UFE2Manager.SetGaugePointsPercent(UFE.p2ControlsScript, staminaOptions.staminaGaugeId, 100);

            UFE2Manager.SetGaugePointsPercent(UFE.p1ControlsScript, parryOptions.parryGaugeId, 100);

            UFE2Manager.SetGaugePointsPercent(UFE.p2ControlsScript, parryOptions.parryGaugeId, 100);
        }
        ...
        private void SetParryOptionsOnButton(ButtonPress button, ControlsScript player)
        {
                ...
                && player.isBlocking == true)
            {
                ...
                UFE2Manager.SetGaugePointsPercent(player, parryOptions.parryGaugeId, 0);
            }
        }
                    ...
                    for (int b = 0; b < lengthB; b++)
                    {
                            ...
                            for (int c = 0; c < lengthC; c++)
                            {
                                switch (gaugeAmountOptions[i].gaugeGainType)
                                {
                                    case GaugeAmountOptions.GaugeGainType.Set:
                                        UFE2Manager.SetGaugePointsPercent(player, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;

                                    case GaugeAmountOptions.GaugeGainType.Add:
                                        UFE2Manager.AddOrSubtractGaugePointsPercent(player, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;
                                }
                            }
                        }
                        ...
                        if (gaugeAmountOptions[i].usePlayerOpponent == true)
                        {
                            ...
                            for (int c = 0; c < lengthC; c++)
                            {
                                ...
                                {
                                    case GaugeAmountOptions.GaugeGainType.Set:
                                        UFE2Manager.SetGaugePointsPercent(player.opControlsScript, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;

                                    case GaugeAmountOptions.GaugeGainType.Add:
                                        UFE2Manager.AddOrSubtractGaugePointsPercent(player.opControlsScript, gaugeAmountOptions[i].gaugeIds[c], gaugeAmountOptions[i].percentAmount);
                                        break;
                                }
                            }
                        }
        ...
        private void SetStaminaGauge(ControlsScript player)
        {
            ...
            if (player.currentMove == null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    UFE2Manager.AddOrSubtractGaugePointsPercent(player, staminaOptions.staminaGaugeId, staminaOptions.passiveStaminaGaugePercentAmount);
        ...
        private void SetParryGauge(ControlsScript player)
        {
            ...
            UFE2Manager.AddOrSubtractGaugePointsPercent(player, parryOptions.parryGaugeId, parryOptions.passiveParryGaugePercentAmount);
        }
        ...
        private void SetParryConfirmOnMove(MoveInfo move, ControlsScript player)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                UFE2Manager.HitPause(player, parryOptions.parryConfirmFreezingTime);

                UFE2Manager.HitPause(player.opControlsScript, parryOptions.parryConfirmFreezingTime);

                if (player.playerNum == 1)
                {
                    if (parryOptions.useKillAllPlayerMovesOnParryConfirmMove == true)
                    {
                        UFE.DelaySynchronizedAction(UFE2Manager.KillAllMovesPlayer1, parryOptions.parryConfirmFreezingTime);
                    }
                ...
                else if (player.playerNum == 2)
                {
                    if (parryOptions.useKillAllPlayerMovesOnParryConfirmMove == true)
                    {
                        UFE.DelaySynchronizedAction(UFE2Manager.KillAllMovesPlayer2, parryOptions.parryConfirmFreezingTime);
                    }
        ...
        private void PlayParryConfirmReactBasicMovePlayer1()
        {
            ...
            /*if (dontPlayIfParryConfirmReactIsPlaying == true
                && (UFE.p1ControlsScript.currentBasicMove == BasicMoveReference.BlockingHighHit
                || UFE.p1ControlsScript.currentBasicMove == BasicMoveReference.BlockingLowHit
                || UFE.p1ControlsScript.currentBasicMove == BasicMoveReference.BlockingCrouchingHit
                || UFE.p1ControlsScript.currentBasicMove == BasicMoveReference.BlockingAirHit)) return;*/

            UFE2Manager.KillAllMoves(UFE.p1ControlsScript);

            BasicMoveInfo basicMoveInfo = null;

            switch (UFE.p1ControlsScript.currentState)
            {
                case PossibleStates.Stand:
                    basicMoveInfo = UFE.p1ControlsScript.MoveSet.basicMoves.blockingHighHit;

                    UFE.p1ControlsScript.currentHitAnimation = parryOptions.standParryConfirmReactBasicMoveAnimationName;

                    UFE.p1ControlsScript.Physics.ResetForces(true, true, true);
                    break;

                case PossibleStates.Crouch:
                    basicMoveInfo = UFE.p1ControlsScript.MoveSet.basicMoves.blockingCrouchingHit;

                    UFE.p1ControlsScript.currentHitAnimation = parryOptions.crouchParryConfirmReactBasicMoveAnimationName;
                ...
                case PossibleStates.BackJump:
                    basicMoveInfo = UFE.p1ControlsScript.MoveSet.basicMoves.blockingAirHit;

                    UFE.p1ControlsScript.currentHitAnimation = parryOptions.jumpParryConfirmReactBasicMoveAnimationName;

                    ForceGrounded(UFE.p1ControlsScript, 2);

                    UFE.p1ControlsScript.Physics.ResetForces(true, true, true);
                    break;
            }

            UFE.p1ControlsScript.currentSubState = SubStates.Stunned;

            UFE.p1ControlsScript.stunTime = (Fix64)parryOptions.parryConfirmReactBasicMoveStunFrames / UFE.config.fps;

            //UFE.p1ControlsScript.MoveSet.PlayBasicMove(basicMoveInfo, UFE.p1ControlsScript.currentHitAnimation);

            UFE.p1ControlsScript.MoveSet.PlayBasicMove(basicMoveInfo);

            if (basicMoveInfo.autoSpeed == true)
            {
                UFE.p1ControlsScript.MoveSet.SetAnimationSpeed(UFE.p1ControlsScript.currentHitAnimation, (UFE.p1ControlsScript.MoveSet.GetAnimationLength(UFE.p1ControlsScript.currentHitAnimation) / UFE.p1ControlsScript.stunTime));
            }
        }
        ...
        private void PlayParryConfirmReactBasicMovePlayer2()
        {
            ...
            /*if (dontPlayIfParryConfirmReactIsPlaying == true
                && (UFE.p2ControlsScript.currentBasicMove == BasicMoveReference.BlockingHighHit
                || UFE.p2ControlsScript.currentBasicMove == BasicMoveReference.BlockingLowHit
                || UFE.p2ControlsScript.currentBasicMove == BasicMoveReference.BlockingCrouchingHit
                || UFE.p2ControlsScript.currentBasicMove == BasicMoveReference.BlockingAirHit)) return;*/

            UFE2Manager.KillAllMoves(UFE.p2ControlsScript);

            BasicMoveInfo basicMoveInfo = null;

            switch (UFE.p2ControlsScript.currentState)
            {
                case PossibleStates.Stand:
                    basicMoveInfo = UFE.p2ControlsScript.MoveSet.basicMoves.blockingHighHit;

                    UFE.p2ControlsScript.currentHitAnimation = parryOptions.standParryConfirmReactBasicMoveAnimationName;

                    UFE.p1ControlsScript.Physics.ResetForces( true, true, true);
                    break;

                case PossibleStates.Crouch:
                    basicMoveInfo = UFE.p2ControlsScript.MoveSet.basicMoves.blockingCrouchingHit;

                    UFE.p2ControlsScript.currentHitAnimation = parryOptions.crouchParryConfirmReactBasicMoveAnimationName;

                    UFE.p2ControlsScript.Physics.ResetForces(true, true, true);
                    break;

                case PossibleStates.NeutralJump:
                case PossibleStates.ForwardJump:
                case PossibleStates.BackJump:
                    basicMoveInfo = UFE.p2ControlsScript.MoveSet.basicMoves.blockingAirHit;

                    UFE.p2ControlsScript.currentHitAnimation = parryOptions.jumpParryConfirmReactBasicMoveAnimationName;

                    ForceGrounded(UFE.p2ControlsScript, 2);

                    UFE.p2ControlsScript.Physics.ResetForces(true, true, true);
                    break;
            }

            UFE.p2ControlsScript.currentSubState = SubStates.Stunned;

            UFE.p2ControlsScript.stunTime = (Fix64)parryOptions.parryConfirmReactBasicMoveStunFrames / UFE.config.fps;

            //UFE.p2ControlsScript.MoveSet.PlayBasicMove(basicMoveInfo, UFE.p2ControlsScript.currentHitAnimation);

            UFE.p2ControlsScript.MoveSet.PlayBasicMove(basicMoveInfo);

            if (basicMoveInfo.autoSpeed == true)
            {
                UFE.p2ControlsScript.MoveSet.SetAnimationSpeed(UFE.p2ControlsScript.currentHitAnimation, (UFE.p2ControlsScript.MoveSet.GetAnimationLength(UFE.p2ControlsScript.currentHitAnimation) / UFE.p2ControlsScript.stunTime));
            }
        }
        ...
        private void CastParryConfirmReactMovePlayer1()
        {
            ...
            {
                case PossibleStates.Stand:
                    UFE2Manager.CastMoveByMoveName(UFE.p1ControlsScript, parryOptions.standParryConfirmReactMoveName);
                    break;

                case PossibleStates.Crouch:
                    UFE2Manager.CastMoveByMoveName(UFE.p1ControlsScript, parryOptions.crouchParryConfirmReactMoveName);
                    break;

                case PossibleStates.NeutralJump:
                case PossibleStates.ForwardJump:
                case PossibleStates.BackJump:
                    UFE2Manager.CastMoveByMoveName(UFE.p1ControlsScript, parryOptions.jumpParryConfirmReactMoveName);
                    break;
            }
        }
        ...
        private void CastParryConfirmReactMovePlayer2()
        {
            ...
            {
                case PossibleStates.Stand:
                    UFE2Manager.CastMoveByMoveName(UFE.p2ControlsScript, parryOptions.standParryConfirmReactMoveName);
                    break;

                case PossibleStates.Crouch:
                    UFE2Manager.CastMoveByMoveName(UFE.p2ControlsScript, parryOptions.crouchParryConfirmReactMoveName);
                    break;

                case PossibleStates.NeutralJump:
                case PossibleStates.ForwardJump:
                case PossibleStates.BackJump:
                    UFE2Manager.CastMoveByMoveName(UFE.p2ControlsScript, parryOptions.jumpParryConfirmReactMoveName);
                    break;
            }
        }
        ...
        private void PlayBlockBreakBasicMove(ControlsScript player, BasicMoveReference basicMove = BasicMoveReference.Idle, bool dontPlayIfBlockBreakIsPlaying = false)
        {
            ...
            UFE2Manager.KillAllMoves(player);
        ...
        private void CastBlockBreakMove(ControlsScript player, BasicMoveReference basicMove = BasicMoveReference.Idle)
        {
            ...
            {
                case BasicMoveReference.BlockingHighHit:
                    UFE2Manager.CastMoveByMoveName(player, blockBreakOptions.standHighBlockBreakMoveName);
                    break;

                case BasicMoveReference.BlockingLowHit:
                    UFE2Manager.CastMoveByMoveName(player, blockBreakOptions.standLowBlockBreakMoveName);
                    break;

                case BasicMoveReference.BlockingCrouchingHit:
                    UFE2Manager.CastMoveByMoveName(player, blockBreakOptions.crouchBlockBreakMoveName);
                    break;

                case BasicMoveReference.BlockingAirHit:
                    UFE2Manager.CastMoveByMoveName(player, blockBreakOptions.jumpBlockBreakMoveName);
                    break;
                ...
                for (int a = 0; a < lengthA; a++)
                {
                    ...
                    switch (characterShakeOptions[i].characterShakeStartMode)
                    {
                        case CharacterShakeOptions.CharacterShakeStartMode.EveryFrame:                          
                            UFE2Manager.ShakeCharacterPosition(player, characterShakeOptions[i].characterShakeOffset);
                            break;
